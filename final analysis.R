...
#adds accent type, accentedness and vowel length in separate columns
epic <- epic %>%
  mutate(accent_type = if_else((tone == "f"), "2",
                               if_else((tone == "r"), "1", "0"))) %>%
  mutate(accented = if_else((accent_type == "0"), 0, 1)) %>%
...
# replicates Ružić (1975)'s tetrasyllable test (section 2.2) 
data %>%
  filter(real == 1) %>%
  filter(accented == 1, of == 4, pos < 3) %>%
  mutate(tetra_initial = if_else(pos == 1, 1, 0)) %>%
  group_by(corpus) %>%
  summarize(rate_initial = mean(tetra_initial), 
            initial = sum(tetra_initial), 
            peninitial = n() - initial) -> tetrasyllables

tetrasyllable_matrix <- xtabs(cbind(initial, peninitial) ~ corpus, data = tetrasyllables)

fisher.test(tetrasyllable_matrix) # for our corpus, I get OR = 1.43, p = 0.012 (still significant, but seemingly less extreme than Ružić's results)

# replicates Ružić (1975)'s trisyllable test (section 2.2)
epic %>% 
  filter(real == 1) %>%
  mutate(marked = if_else(lag(accented) == 1 & lead(accented) == 1, 1, 0),  # with reference to ultima of first trisyllable
         unmarked = if_else(lag(accented, 2) == 1 & lead(accented, 2) == 1, 1, 0)) |>
  filter(of == 3 & pos == 3 & lead(of) == 3 & position == 7) |>  # get consecutive trisyllables in the right position
  count(marked, unmarked) -> trisyllable_test_epic # 68 in the unmarked order, 36 in the marked order (1.9x unmarked)
...
prose %>%
  ...
  mutate(marked = if_else(lag(accented) == 1 & lead(accented) == 1, 1, 0),  # with reference to ultima of first trisyllable
         unmarked = if_else(lag(accented, 2) == 1 & lead(accented, 2) == 1, 1, 0)) |>
  filter(of == 3 & pos == 3 & lead(of) == 3 & position == 7) |>  # get consecutive trisyllables in the right position
  count(marked, unmarked) -> trisyllable_test_prose # 15 in the unmarked order, 13 in the marked order (1.2x unmarked)
...
# percentage of syllable types in weak positions (section 6.2)
data %>%
  filter(!(position == "4" | position == "10")) %>%
  filter(corpus == "epic" & real == 1) %>%
  group_by(accent_type, weight, odd) %>%
